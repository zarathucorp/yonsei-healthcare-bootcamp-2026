...
R Console에서 순서대로 실행해 `.Renviron` 파일을 연다.
...
## Posit Cloud 4 - 세션 재시작

`.Renviron`은 세션을 다시 시작해야 반영된다.

:::::: columns
:::: {.column width="52%"}
::: large
- 우측 상단 **`...` 메뉴 → Relaunch Project**를 누른다.
- 재시작 후 Console에서 확인:
:::
...
## Posit Cloud 5 - config.toml 만들기

Codex를 설치하면 `.codex` 폴더가 **홈 폴더**(`/home/사용자ID`)에 이미 생긴다. 새로 만들 필요 없다.

:::::: columns
:::: {.column width="46%"}
::: large
- Files 창에서 **홈 폴더**로 이동 → **More → Show Hidden Files** → `.codex`가 보인다. (프로젝트 폴더 `/cloud/project` 아님)
- R Console에서 편집기로 열어 저장:
:::

``` r
file.edit("/home/r150450/.codex/config.toml")
```
::::
...
## Posit Cloud 6 - config.toml 내용

`config.toml`을 열어 아래 내용을 붙여넣고 저장한다.
