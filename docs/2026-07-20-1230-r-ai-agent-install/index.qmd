...
usethis::edit_r_environ()     # LITELLM_API_KEY=전달받은 키 저장
```

`config.toml`은 Console에서 폴더를 만들고 편집기로 연다.

``` r
dir.create("~/.codex", showWarnings = FALSE)
file.edit("~/.codex/config.toml")   # 편집기에서 열림 → 아래 내용 저장
```

::: highlight-box
붙여넣을 `config.toml` 내용은 **Mac 6 / Windows 6 슬라이드와 동일**하다. 저장 후 세션을 재시작한다(우측 상단 새로고침).
:::
